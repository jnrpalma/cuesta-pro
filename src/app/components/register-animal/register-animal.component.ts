...
export class RegisterAnimalComponent implements OnInit {
  ...
  toggleForm() {
    ...
    this.formButtonLabel = this.showForm
      ? "Ocultar Formulário de Cadastro animal"
      : "Formulário de Cadastro animal";
  }
  ...
  cadastrar() {
    ...
    this.animalService
      ...
      .then((exists) => {
        if (exists) {
          this.poNotification.error(
            "Já existe um animal cadastrado com este ID."
          );
          this.isLoading = false;
          return;
        }

        const animalsToRegister = Array.from(
          { length: this.animal.quantity },
          () => ({ ...this.animal })
        );

        const promises = animalsToRegister.map((animal) =>
        ...
        Promise.all(promises)
          .then(() => {
            this.poNotification.success(
              "Todos os animais foram cadastrados com sucesso!"
            );
            this.limparFormulario();
          })
          .catch((error) => {
            console.error("Erro ao cadastrar animais:", error);
      ...
      .catch((error) => {
        console.error("Erro ao verificar duplicidade de ID:", error);
        this.poNotification.error("Erro ao verificar duplicidade de ID.");
        this.isLoading = false;
      });
  }
